...
def test_are_times_contiguous(mocker, manager_class: DatasetManager):
    ...
    assert dm.are_times_contiguous(contig, expected_delta=expected_delta)
    # Check a single time -- one good, one not
    check1 = [contig[0], contig[1]]
    check2 = [contig[0], contig[2]]
    assert dm.are_times_contiguous(check1, expected_delta=expected_delta)
    assert not dm.are_times_contiguous(check2, expected_delta=expected_delta)
    # Check a set of times that skips a day
    week_ahead_dt = contig[-1] + pd.Timedelta(days=7)
    week_gap = contig.union([week_ahead_dt])
    assert not dm.are_times_contiguous(week_gap, expected_delta=expected_delta)
    ...
    assert not dm.are_times_contiguous(out_of_order, expected_delta=expected_delta)
    ...
    assert dm.are_times_contiguous(three_and_four_day_updates, expected_delta=expected_delta)
    ...
    assert not dm.are_times_contiguous(five_day_updates, expected_delta=expected_delta)
