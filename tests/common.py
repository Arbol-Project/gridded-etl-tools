...
def patched_root_stac_catalog(self):
    return {
        "id": f"{self.data_host}_data_catalog_test",
        "type": "Catalog",
        "title": f"{self.data_host} Data Catalog - test",
        "stac_version": "1.0.0",
        "description": f"This catalog contains all the data uploaded by {self.data_host} that has been issued STAC-compliant metadata. The catalogs and collections describe single providers. Each may contain one or multiple datasets. Each individual dataset has been documented as STAC Items."
        }
