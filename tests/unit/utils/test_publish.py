...
class TestPublish:
    ...
    def test_post_parse_quality_check(manager_class, fake_original_dataset, mocker):
        ...
        dm._get_prod_update_ds = mock.Mock(return_value=fake_original_dataset)
        ...
        def check_written_value(dataset1, dataset2, threshold, checks):
            assert dataset1 is dm._raw_file_to_dataset.return_value.where.return_value
            assert dataset2 is dm._get_prod_update_ds.return_value
        ...
        dm.set_key_dims.assert_called_once_with()
        dm._filter_search_space.assert_called_once_with(dm._get_prod_update_ds.return_value)
        ...
        dm._raw_file_to_dataset.assert_called_with(
            random.choice(dm._filter_search_space(dm._get_prod_update_ds.return_value))
        )
    ...
    def test_post_parse_quality_check_constrained(manager_class, fake_original_dataset, mocker):
        ...
        random.choice.assert_called_with(dm._filter_search_space())
        assert dm._raw_file_to_dataset.call_count == 5
        dm._raw_file_to_dataset.assert_called_with(
            random.choice(dm._filter_search_space(dm._get_prod_update_ds.return_value))
        )
    ...
    def test_post_parse_quality_check_warn(manager_class, fake_original_dataset, mocker):
        ...
        dm._raw_file_to_dataset = mock.Mock(return_value=fake_original_dataset)
        dm._get_prod_update_ds = mock.Mock(return_value=fake_original_dataset)
        dm._filter_search_space = mock.Mock(return_value=["Do_you_like_jazz.mp3"])
