...
### GRIBs are already checked into that root. These GRIBs are copied into the generated input directories to
...
### to erase everything that was generated (everything in the default root except for the input GRIBs).
### 

import os, datetime, pytest, xarray, shutil
# import local functions common to all pytests
from ..common import *
...
def simulate_file_download(root, initial_input_path, appended_input_path):
    """
    Copies the default input GRIBs into the default input paths, simulating a download of original data. Later, the input directories will be
    ...
    print(f"Simulated downloading input GRIB files")
...
def setup_and_teardown_per_test(mocker, request, initial_input_path, appended_input_path,
    ...
    mocker.patch("etls.dataset_manager.DatasetManager.json_key", patched_json_key)
    mocker.patch("etls.managers.chirps.CHIRPS.collection", return_value=f"CHIRPS_test")
    mocker.patch("etls.dataset_manager.DatasetManager.zarr_json_path", patched_zarr_json_path)
    mocker.patch("etls.dataset_manager.DatasetManager.default_root_stac_catalog", patched_root_stac_catalog)
    mocker.patch("etls.dataset_manager.DatasetManager.ipns_publish", empty_ipns_publish)
...
def test_initial(request, mocker, manager_class, heads_path, test_chunks, initial_input_path, root):
    ...
    if manager.json_key() not in manager.ipns_key_list():
        mocker.patch("etls.dataset_manager.DatasetManager.ipns_publish", offline_ipns_publish)
...
def test_append_only(mocker, request, manager_class, heads_path, test_chunks, appended_input_path, root):
    ...
    manager = manager_class(custom_input_path=appended_input_path)
    manager.HASH_HEADS_PATH = heads_path
    manager.zarr_chunks = {}
    # Overriding the default time chunk to enable testing chunking with a smaller set of times
    manager.requested_dask_chunks = test_chunks
