...
def pytest_addoption(parser):
    ...
    Options are global whether they are defined in the root tests/ directory or in a subdirectory like tests/CHIRPS.
    Therefore, option names in CHIRPS/conftest.py and prism_zarr/conftest.py cannot be the same.

    For example, if you wanted an option like "time_chunk", it would either have to be defined here once and apply to
    both CHIRPS and PRISM or would have to have to be defined with a different name in each, like "CHIRPS_time_chunk" and
    "prism_time_chunk".
