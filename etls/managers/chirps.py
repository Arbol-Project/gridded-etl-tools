...
import glob, datetime, pathlib, datetime, re, requests
import xarray as xr
from etls.dataset_manager import DatasetManager
...
class CHIRPS(DatasetManager):
    ...
    def __init__(self, *args,
                # 0.05 dataset size is our default and its full shape is time: ~15000, latitude: 2000, longitude: 7200
    ...
    @property
    def tags(self):
        """Tags for data to enhance filtering of STAC metadata"""
        return ["Precipitation"]
    ...
    def dataset_start_date(self):
        """Abstractmethod to force instantiation of dataset_start_date property in each child class"""
        ...
    ...
    def dataset_download_url(self) -> str:
        ...
        return f"https://data.chc.ucsb.edu/products/CHIRPS-2.0"
    ...
    def update_local_input(self, date_range: list[str] = None, *args, **kwargs) -> bool:
        """
        ...
        if not date_range:
            try:
                ...
                self.info(f"Because no STAC metadata found, starting file search from the dataset beginning of {self.dataset_start_date}")
    ...
    def set_zarr_metadata(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        keys_to_remove = ['Conventions', 'history', 'version', 'date_created', \
...
class CHIRPSFinal(CHIRPS):
    ...
    @property
    def dataset_start_date(self):
        return datetime.datetime(1981, 1, 1, 0, 0)

    @classmethod
    def name(cls) -> str:
        """Name used to refer to the dataset in IPFS / IPNS"""
...
class CHIRPSFinal25(CHIRPSFinal):
    ...
    def __init__(self, *args, **kwargs):
        ...
        chunks = dict(  requested_dask_chunks={"time": 500, "latitude": 40, "longitude": -1}, # 115 MB
