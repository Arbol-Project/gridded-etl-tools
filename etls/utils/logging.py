...
class Logging(Attributes):
    ...
    def log_to_file(cls,
            ...
            time_format: str = "%Y/%m/%d %H:%M") -> logging.Handler:
        """
        ...
        if logging.getLogger().hasHandlers():
            for handler in logging.getLogger().handlers:
                ...
                if isinstance(handler, logging.handlers.WatchedFileHandler) and handler.level == level and \
                   pathlib.Path(handler.baseFilename).resolve() == pathlib.Path(path).resolve():
    ...
    @classmethod
    def log_to_console(cls, level: str = logging.INFO, log_format: str = "%(levelname)-8s <%(name)s> %(message)s"):
        """
