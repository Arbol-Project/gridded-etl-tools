...
import ipldstore, dag_cbor, requests
...
class IPFS(ABC):
    """
    Abstract base class holding methods to instantiate and interact with an IPFS client
    """
    def __init__(self,
    ...
    @classmethod
    def _host(cls) -> str:
        """The URL hosting your IPFS instance"""
        return "http://127.0.0.1:5001" # NOTE specify your host URL here if not your local machine
    ...
    def ipfs_get(self, cid: str) -> dict:
        ...
        """
        res = self.ipfs_session.post(self._host() + "/api/v0/block/get", timeout = self._default_timeout, params={"arg": str(cid)})
    ...
    def ipfs_put(self,
        ...
        """
        res = self.ipfs_session.post(self._host() + "/api/v0/dag/put",
                        params={"store-codec": "dag-cbor",
    ...
    def ipns_resolve(self, key: str) -> str:
        ...
        """
        res = self.ipfs_session.post(self._host() + "/api/v0/name/resolve", timeout = self._default_timeout, params = {"arg" : key})
        res.raise_for_status()
        return res.json()['Path'][6:] # 6: shaves off leading '/ipfs/'


    def ipns_publish(self,
                    key: str,
                    cid: str,
        ...
        res = self.ipfs_session.post(self._host() + "/api/v0/name/publish", timeout = self._default_timeout, params={
            "arg": cid,
            "key" : key,
    ...
    def ipns_key_list(self) -> dict:
        ...
        """
        ipns_key_hash_dict = {}
        for name_hash_pair in self.ipfs_session.post(self._host() + "/api/v0/key/list", timeout = self._default_timeout).json()['Keys']:
    ...
    def ipns_generate_name(self, key: str = None) -> str:
        ...
        if key not in self.ipns_key_list():
            res = self.ipfs_session.post(self._host() + "/api/v0/key/gen", timeout = self._default_timeout, params={"arg" : key, "type" : "rsa"})
    ...
    def check_stac_on_ipns(self, key: str) -> bool:
        ...
        try:
            ...
            self.info(f"No existing STAC-compliant object found for {key}. Creating new object, pushing it to IPFS, and populating any linked collections/catalogs")
            exists = False
    ...
    def ipld_mapper(self) -> ipldstore.IPLDStore:
        """
        Get an IPLD mapper by delegating to ipldstore.get_ipfs_mapper, passing along the IPFS chunker value request if `self.requested_ipfs_chunker`
        has been set. The mapper can be passed to `xarray.open_zarr` to open a Zarr on the IPLD datastore.

        Returns
        -------
        ipldstore.IPLDStore
            An ipldstore.IPLDStore (IPLD mapper) object
        """
        if self.requested_ipfs_chunker:
            mapper = ipldstore.get_ipfs_mapper(chunker=self.requested_ipfs_chunker)
        else:
            mapper = ipldstore.get_ipfs_mapper()
        self.log(f"IPFS chunker is {mapper._store._chunker}")
        return mapper
