...
import os, gc, pathlib, natsort, datetime, re, ftplib, io, json
import pandas as pd
import numpy as np
import xarray as xr

from abc import abstractmethod
from etls.utils.attributes import Attributes
...
class Convenience(Attributes):
    ...
    def zarr_json_path(self) -> pathlib.Path:
        ...
        """
        return self.SOURCE_FILE_PATH.parent / f"{self.name()}_zarr.json"
    ...
    def output_path(self, omit_root: bool = False) -> str:
        ...
        if self.custom_output_path is not None:
            ...
            if "date range" in stac_metadata["properties"]:
                date = self.get_metadata_date_range()["end"]
            elif "updated" in stac_metadata["properties"]:
                date = datetime.datetime.fromisoformat(stac_metadata["properties"]["updated"])
            return self.get_folder_path_from_date(date, omit_root)
        else:
            self.info("No custom output path specified and stac metadata not available to provide a date-based path. \
                Writing to default output directory")
            return self.OUTPUT_ROOT / self.relative_path()
    ...
    def get_metadata_date_range(self) -> dict:
        ...
        """
        stac_metadata = self.load_stac_metadata()
        return {"start": datetime.datetime.strptime(stac_metadata["properties"]["date range"][0],'%Y%m%d%H'),
                "end": datetime.datetime.strptime(stac_metadata["properties"]["date range"][1],'%Y%m%d%H')}
    ...
    def get_date_range_from_file(self, path: str, backend_kwargs: dict = None) -> tuple[datetime.datetime, datetime.datetime]:
        """
    ...
    def date_range_to_string(self, date_range: tuple) -> tuple[str, str]:
        ...
        -------
        tuple
            A tuple of Isoformatted (w/ hours) start and end dates of a date range

        """
        return (datetime.datetime.strftime(date_range[0],'%Y%m%d%H'), datetime.datetime.strftime(date_range[1],'%Y%m%d%H'))
    ...
    def sync_ftp_files(self, server: str, directory_path: str, file_match_pattern: str, include_size_check: bool = False):
        """
            ...
            for file_name in ftp.nlst():
                if re.match(file_match_pattern, file_name):
                    ...
                    if local_file_path.exists():
                        ...
                        if modification_time <= local_file_mtime and (not include_size_check or remote_file_size == local_file_size):
                            self.debug("local file {} does not need updating".format(local_file_path))
                            retrieve = False
                        elif modification_time > local_file_mtime:
                            self.debug("file {} local modification time {} less than remote modification time {}".format(
                                local_file_path, local_file_mtime.strftime("%Y/%m/%d"), modification_time.strftime("%Y/%m/%d")))
                        else:
    ...
    def bbox_coords(self, dataset: xr.Dataset) -> tuple[float, ...]:
        """
        ...
        """
        return round(float(dataset.longitude.values.min()),self.bbox_rounding_value), round(float(dataset.latitude.values.min()),self.bbox_rounding_value), \
            round(float(dataset.longitude.values.max()),self.bbox_rounding_value), round(float(dataset.latitude.values.max()),self.bbox_rounding_value)
