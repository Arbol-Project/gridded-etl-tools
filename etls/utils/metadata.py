...
class Metadata(Convenience, IPFS):
    ...
    def default_stac_collection(self) -> dict:
        ...
        default_stac_collection = {
            ...
            "providers" : [ {
                "name": "dClimate",
                "description": ( "A transparent, decentralized marketplace that allows participants to buy and share data"
            ...
            "summaries" : {
                "proj:epsg" : self.metadata["coordinate reference system"]
            },
            "license" : self.metadata["license"]
        }

        return default_stac_collection


    @classmethod
    def default_root_stac_catalog(cls) -> dict:
        """
        Default metadata template for the dClimate root Catalog
        ...
        """
        return {
        "id": "dClimate_data_catalog",
        "type": "Catalog",
        "title": "dClimate Data Catalog",
        "stac_version": "1.0.0",
        "description": "This catalog contains all the data uploaded to dClimate that has been issued STAC-compliant metadata. The catalogs and collections describe single providers. Each may contain one or multiple datasets. Each individual dataset has been documented as STAC Items."
        }
    ...
    def create_root_stac_catalog(self):
        """
        Prepare a root catalog for dClimate if it doesn't already exist.
        """
        ...
        if not root_catalog_exists:
            ...
            self.info("Publishing root dClimate STAC Catalog")
            root_catalog_ipns_name = self.ipns_generate_name(key=root_catalog["title"])
            root_catalog["links"] = [{"rel" : "root", "href" : root_catalog_ipns_name, "type" : "application/json", "title" : 'dClimate root catalog'},
                {"rel" : "self", "href" : root_catalog_ipns_name, "type" : "application/json", "title" : 'dClimate root catalog'}]
            self.ipns_publish(root_catalog["title"], self.ipfs_put(self.json_to_bytes(root_catalog)))
        else:
            self.info("Root dClimate STAC Catalog already exists, building collection")
            ...
    ...
    def create_stac_collection(self, dataset: xr.Dataset, rebuild: bool = False):
        ...
        if rebuild or not self.check_stac_on_ipns(self.collection()):
            if rebuild:
                self.info("Collection rebuild requested. Creating new collection, pushing it to IPFS, and populating the main catalog")
            ...
            stac_coll["extent"]["temporal"]["interval"] = [[self.numpydate_to_py(dataset.time.values.min()).isoformat()+'Z',\
            ...
            root_catalog, root_catalog_ipns_name, _ = self.ipns_retrieve_object(self.default_root_stac_catalog()["title"])
            if not any(stac_coll["title"] in link_dict.values() for link_dict in root_catalog["links"]):
                self.info("Appending collection link to root dClimate STAC Catalog")
                root_catalog["links"].append({"rel" : "child", "href" : coll_ipns_name, "type" : "application/json", "title" : stac_coll["title"]})
                self.ipns_publish(root_catalog["title"], self.ipfs_put(self.json_to_bytes(root_catalog)))
            # Add links and publish this collection
            for link_dict in stac_coll["links"]:
                if link_dict["rel"] == "self":
                    link_dict["href"] = coll_ipns_name
            stac_coll["links"] = stac_coll["links"] + [{"rel" : "root", "href" : root_catalog_ipns_name, "type" : "application/json", "title" : 'dClimate root catalog'},
                    {"rel" : "parent", "href" : root_catalog_ipns_name, "type" : "application/json", "title" : 'dClimate root catalog'}]
            self.ipns_publish(self.collection(), self.ipfs_put(self.json_to_bytes(stac_coll)))
        else:
            self.info(f"Found existing STAC Collection for this dataset, skipping creation")
    ...
    def create_stac_item(self, dataset: xr.Dataset, dataset_hash: str):
        ...
        """
        self.info(f"Creating STAC Item")
        ...
        properties_dict["array_size"] = {"latitude" : dataset.latitude.size, "longitude" : dataset.longitude.size, "time" : dataset.time.size}
        # Set up date items in STAC-compliant style
        properties_dict["start_datetime"] = self.numpydate_to_py(dataset.time.values[0]).isoformat()+'Z'
        properties_dict["end_datetime"] = self.numpydate_to_py(dataset.time.values[-1]).isoformat()+'Z'
        properties_dict["updated"] = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()[:-13]+'Z'
    ...
    def zarr_md_to_stac_format(self, dataset: xr.Dataset) -> dict:
        ...
        zarr_attrs = {"missing value", "dtype", "preferred_chunks", "Conventions", "spatial resolution", "spatial precision", "temporal resolution", "update cadence", \
            "unit of measurement", "tags", "standard name", "long name", "date range", "dataset description", "dataset download url", "created", "updated",
            "finalization date", \
            "analysis data download url", "reanalysis data download url", \
            "input history cid", "input history JSON documentation"}
        all_md = {**dataset.attrs, **dataset.encoding, **dataset[self.data_var()].encoding}
        all_md = {key: all_md[key] for key in zarr_attrs if key in all_md}
        rename_dict = {"preferred_chunks" : "Zarr chunk size", "missing value" : "Fill value", "Conventions" : "CF convention"}
    ...
    def register_stac_item(self, stac_item: dict):
        ...
        """
        self.info(f"Registering STAC Item in IPFS and its parent STAC Collection")
        # Generate variables of interest
        stac_coll, coll_ipns_name, _ = self.ipns_retrieve_object(str(self.collection()))
        # Register links
        stac_item["links"].append({"rel": "parent", "href": coll_ipns_name, "type": "application/geo+json", "title": stac_coll["title"]})
        try:
            # Check if an older version of the STAC Item exists and if so create a "previous" link for them in the new STAC Item
            old_stac_item, item_ipns_name, old_item_ipfs_hash = self.ipns_retrieve_object(self.json_key())
            stac_item["links"].append({
                "rel" : "prev",
                "href" : str(old_stac_item["assets"]["zmetadata"]["href"].set(base=self._default_base)), # convert CID object back to hash str
                "metadata href" : {'/' : old_item_ipfs_hash},
                "type" : "application/geo+json",
                "title" : stac_item["assets"]["zmetadata"]["title"]
                })
            self.info(f"Updating 'previous' link in dataset's STAC Item")
        except (KeyError, TimeoutError):
            # Otherwise create an IPNS name for your new dataset
            self.info(f"No previous STAC Item found")
        ...
        self.info(f"Pushing STAC Item to IPFS and linking to IPNS")
        ...
        if any(stac_item["assets"]["zmetadata"]["title"] in link_dict["title"] for link_dict in stac_coll["links"] if "title" in link_dict.keys()):
            self.info(f"Found existing STAC Item in STAC Collection")
            ...
        else:
            self.info(f"No existing STAC Item found in this dataset's parent collection, inserting a child link")
    ...
    def update_stac_collection(self, dataset: xr.Dataset):
        ...
        dataset : xr.Dataset
            The dataset being published
        """
        self.info(f"Updating dataset's parent STAC Collection")
    ...
    def stac_collection_data_summaries(self, dataset: xr.Dataset) -> dict:
        """
        Descriptive summary statistics for the data variable in an N-Dimensional dataset. To be displayed in the parent STAC Collection for said dataset.

        NOTE This function is optional and unused in the current implementation of the architecture as it's very computationally expensive for modest gains.
        NOTE This assumes only one data variable per dataset.
        ...
        -------
        dict
            Summary descriptive statistics for the input dataset's data variable

        """
        summaries = {
                "minimum" : round(float(dataset.u100.values.min()),8),
                "maximum" : round(float(dataset.u100.values.max()),8),
                "mean" : round(float(dataset.u100.values.mean()),8),
                "standard deviation" : round(float(dataset.u100.values.std(),8))
        }
        return summaries
    ...
    def only_update_metadata(self):
        ...
        This function is called via a `--only-metadata` flag to generate_zarr.py.
        """
    ...
    def load_stac_metadata(self, key: str = None) -> str | dict:
        ...
        try:
            ...
            self.info("STAC metadata requested but no STAC object found at the provided key")
            return {}
    ...
    def merge_in_outside_metadata(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        # Populate creation date and date range from stac metadata, if it exists, and the input dataset if not
        try:
            existing_stac_metadata = self.load_stac_metadata()
            dataset.attrs["created"] = existing_stac_metadata["properties"]["created"]
            dataset.attrs["date range"] = (
                existing_stac_metadata["properties"]["date range"][0], datetime.datetime.strftime(self.get_date_range_from_dataset(dataset)[1],'%Y%m%d%H'))
        except (KeyError, TimeoutError):
            dataset.attrs["created"] = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()[:-13]+'Z'
    ...
    def remove_unwanted_fields(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        for coord in ["latitude","longitude"]:
            ...
            dataset[coord].encoding["compressor"] = None
        dataset[self.data_var()].encoding.pop("filters",None)
        dataset[self.data_var()].encoding["compressor"] = None

        return dataset


    def set_zarr_spatial_ref(self, dataset: xr.Dataset) -> xr.Dataset:
        """
        Create the spatial reference information for a Zarr the first time it's written.
        This must *only* be run when creating a dataset as spatial_ref variables in updates will cause updates using the `region` flag to fail. 

        Parameters
        ----------
        dataset : xarray.Dataset
            The dataset being published, pre-spatial reference update

        Returns
        -------
        dataset : xarray.Dataset
            The dataset being published, after spatial reference update

        """
        # include spatial reference information in the main attributes and the coordinate attributes, as per GeoZarr spec
        if "EPSG" in dataset.attrs["coordinate reference system"]:
            dataset.rio.write_crs(dataset.attrs["coordinate reference system"], inplace=True) # auto-generates CRS info under name "crs"
        else: # ERA5 and some other dataset lack a traditional CRS, we default to EPSG:4326 (WGS84)in these cases
            dataset.rio.write_crs("epsg:4326", inplace=True)
        dataset[self.data_var()].encoding["grid_mapping"] = 'spatial_ref'
        dataset["latitude"].encoding["grid_mapping"] = 'spatial_ref'
        dataset["longitude"].encoding["grid_mapping"] = 'spatial_ref'
