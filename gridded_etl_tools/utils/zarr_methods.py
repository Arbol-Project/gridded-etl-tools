...
class Publish(Transform, Metadata):
    ...
    def parse(self) -> bool:
        ...
        with LocalCluster(
            ...
            # instances = [instance for instance in cluster._instances if cluster.status.name != "closed"]
            # if len(instances) > 1:
            #     for instance in instances:
            #         if instance is not cluster:
            #             instance.close()

            # instances = [instance for instance in cluster._instances if cluster.status.name != "closed"]
            # if len(instances) > 1:
            #     raise RuntimeError("Previous Dask clusters are not cleaning up after themselves")
