...
def _compute_affine_transform(y_coords: np.ndarray, x_coords: np.ndarray) -> list[float]:
    ...
    Uses median spacing for robustness against floating-point rounding.
    """
...
def build_spatial_attrs(dataset: xr.Dataset, spatial_dims: list[str]) -> dict:
    ...
    -------
    dict
        Dict of ``spatial:*`` keys. Empty if spatial dims are not found.
    """
    ...
    try:
        has_coords = y_dim in dataset.coords and x_dim in dataset.coords
    except TypeError:
        return {}
    if not has_coords:
